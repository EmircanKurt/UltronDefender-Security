    ...
    public class EtwProcessMonitorService : IEtwProcessMonitorService
    {
        ...
        public void Start()
        {
            lock (_lock)
            {
                ...
                try
                {
                    // WMI Win32_ProcessStartTrace (Event Tracing Provider for Win32 Processes)
                    var query = new WqlEventQuery("__InstanceCreationEvent", TimeSpan.FromSeconds(1), "TargetInstance ISA 'Win32_Process'");
                    _processStartWatcher = new ManagementEventWatcher(query);
                    _processStartWatcher.EventArrived += OnProcessStarted;
                    _processStartWatcher.Start();
                    _logger?.LogInformation("Real-Time Process Start Trace Provider hooked successfully.");
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning(ex, "WMI Process Trace hook failed (Non-admin or WMI restricted). Falling back to hybrid polling monitor.");
        ...
        private void OnProcessStarted(object sender, EventArrivedEventArgs e)
        {
            try
            {
                if (e.NewEvent["TargetInstance"] is ManagementBaseObject targetInstance)
                {
                    ...
                    // Komut Satırını Anında Değerlendir
